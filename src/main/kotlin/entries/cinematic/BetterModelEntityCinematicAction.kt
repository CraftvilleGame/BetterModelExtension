...
class BetterModelEntityCinematicAction(
    private val player: Player,
    private val entry: BetterModelEntityCinematicEntry,
) : SimpleCinematicAction<BetterModelEntityRecordedSegment>() {
    ...
    override val segments: List<BetterModelEntityRecordedSegment>
        get() = entry.segments
    ...
    override suspend fun startSegment(segment: BetterModelEntityRecordedSegment) {
    ...
    override suspend fun tickSegment(segment: BetterModelEntityRecordedSegment, frame: Int) {
        ...
        segment.animations.forEach {
            ...
            val tracker = baseEntity()?.tracker() ?: return

            if (stop) {
                if (animation.isNotEmpty()) tracker.stopAnimation(animation)
            } else {
                tracker.animate(animation, it.animationSettings.toModifier())
            }
        }
    ...
    private fun baseEntity(): BetterModelEntity? {
        if (entity is NamedBetterModelEntity)
            return (entity as NamedBetterModelEntity).getEntity()
        if (entity is BetterModelEntity)
            return entity as BetterModelEntity
        return null
    }

    override suspend fun stopSegment(segment: BetterModelEntityRecordedSegment) {
